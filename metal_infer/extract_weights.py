#!/usr/bin/env python3
"""
extract_weights.py — Extract all non-expert weights from Qwen3.5-35B-A3B-4bit
or Qwen3.5-397B-A17B into a single binary file that the C inference engine can mmap.
...
Usage:
    python extract_weights.py [--model PATH] [--output DIR] [--model-size 35B|397B]
"""
...
def main():
    ...
    parser.add_argument('--model-size', type=str, choices=['35B', '397B'], default=None,
                        help='Model size (35B or 397B). Auto-detected from config.json if not set.')
    args = parser.parse_args()

    model_path = Path(args.model)

    # Auto-detect model size from config.json, fall back to --model-size (default 35B)
    model_size = args.model_size
    config_json_path = model_path / 'config.json'
    if model_size is None and config_json_path.exists():
        try:
    ...
    # Layer type map
    layer_types = []
    for i in range(selected_config["num_hidden_layers"]):
        if (i + 1) % 4 == 0:
